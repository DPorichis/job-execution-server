...
Here is a short sum-up of how each action on the buffer is performed:
	...
	- Poll: The function locks the mutex to ensure the state of the buffer stays the same, and goes over all queued jobs, constructing a representation string for each. It then unlocks the mutex, constructs a continuous string containing all the representations stored earlier, and returns it.
